...
list2texcom <- function(x,titre="") {
  paste0(length(x)," \\pdfcomment[icon=Note]{",
         titre," \\textCR\\textCR ",
...
cat_etab_slide <- function(etab) {
  ...
  cat(sep = '',"\n\n",
    r2tex("etabpage[section]",
          ...
          r2tex("makeinfo",info$swurl,info$type,info$lfurl,info$uai,info$wdurl,etab$Académie,info$dans.tdb,info$dans.evol,info$hors.norm))
  )
...
esr.etab[esr.etab$UAI=="0352305R",]$url.siteweb <- "https://www.rennes-sb.fr"
