...
\newlength{\secbandheight}\setlength{\secbandheight}{0.05\paperheight}
\newlength{\numxshift}\setlength{\numxshift}{-3cm}
\newlength{\titxshift}\setlength{\titxshift}{0.20\paperwidth}
\def\numscale{8}
\def\titscale{3}
...
\titleformat{\chapter}
  ...
  {\begin{tikzpicture}[remember picture,overlay]
    ...
    \node (bandeauTop) at (current page.north)
      [draw, anchor=north, 
      minimum width=\paperwidth, minimum height=\chapbandheight, 
      color=bandcol, fill=bandcol] {};
    \node (num) at ($ (bandeauTop.south east) + (\numxshift,-0.1cm) $)
      [anchor=south east, color=white, scale=\numscale, inner sep=0] {\chapterlabel};
    \node (tit) at ($ (bandeauTop.west) + (\titxshift,0) $)
      [anchor=west, scale=\titscale, color=white] {#1};
   \end{tikzpicture}
  }
...
\newcommand\verticalinfo[3]{
  \begin{tikzpicture}[remember picture,overlay]
    \node (combandeau) at (current page.north west) 
      ...
      minimum width=\verticalinfowidth, minimum height=\paperheight, 
      color=bandcol, fill=bandcol] {};
    \node (titre) at ($ (combandeau.north) + (0,-\chapbandheight) $)
